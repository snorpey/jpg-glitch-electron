...
export class PanZoom {
	constructor ( containerEl, targetEl, opts = { } ) {
		...
		this.containerEl.addEventListener( 'mousedown', this.pointerPressed );
		this.containerEl.addEventListener( 'touchstart', this.pointerPressed );
		this.containerEl.addEventListener( 'wheel', this.wheelTurned );
		this.containerEl.addEventListener( 'dblclick', this.animateToCenter.bind( this ) );

		window.addEventListener( 'resize', this.resized.bind( this ) );

		this.targetEl.style.WebkitTransformOrigin = targetEl.style.transformOrigin = '0 0';

		elementResizeEvent( this.targetEl, this.updateDimensions.bind( this ) );
		elementResizeEvent( this.containerEl, this.updateDimensions.bind( this ) );
	...
	updateValues ( newX, newY, newScale ) {
		...
		this.updateAnimationFrameId = requestAnimationFrame( this.update.bind( this ) );
	}
	...
	firstPointerPressed ( event ) {
		...
		document.addEventListener( 'mousemove', this.pointerMoved );
		document.addEventListener( 'mouseup', this.pointerReleased );
		
		document.addEventListener( 'touchmove', this.pointerMoved );
		document.addEventListener( 'touchend', this.pointerReleased );
	}
	...
	pointerReleased ( event ) {
		...
		if ( this.active ) {
			...
			document.removeEventListener( 'mousemove', this.pointerMoved );
			document.removeEventListener( 'mouseup', this.pointerReleased );

			document.removeEventListener( 'touchmove', this.pointerMoved );
			document.removeEventListener( 'touchend', this.pointerReleased );
		}
	}
